 ...
 * 
 * Auto-synced on 2025-10-03T06:38:10.541Z
 */
/**
 * Master
...
const relationships = {
  PLAYED_IN: {
    ...
    properties: {
      ...
      goals: { type: 'integer', default: 0 },                    // Goals scored in this match
      assists: { type: 'integer', default: 0 },                  // Assists made in this match
      fantasyPoints: { type: 'integer', default: 0 },            // Fantasy points earned in this match
      minutes: { type: 'integer', default: 0 },                  // Minutes played in this match
      yellowCard: { type: 'boolean', default: false },           // Whether player received yellow card
      redCard: { type: 'boolean', default: false },              // Whether player received red card
      cleanSheet: { type: 'boolean', default: false },           // Whether player kept clean sheet
      manOfMatch: { type: 'boolean', default: false },           // Whether player was man of the match
      
      // === LOCATION PROPERTIES ===
      isHome: { type: 'boolean', required: false },              // Whether match was played at home
      location: { type: 'string', required: false },             // Opposition team name (location context)
      
      // === TEMPORAL ANALYSIS PROPERTIES (Optional - computed on-demand) ===
      // Consecutive match streak properties
      streakType: { type: 'string', required: false },           // Type of streak: 'goals', 'assists', 'clean_sheets', 'appearances'
      streakLength: { type: 'integer', required: false },        // Length of current streak
      streakStartDate: { type: 'date', required: false },        // Start date of current streak
      streakEndDate: { type: 'date', required: false },          // End date of current streak
      
      // Milestone achievement properties
      milestoneType: { type: 'string', required: false },        // Type of milestone: '100_goals', '50_appearances', etc.
      milestoneValue: { type: 'integer', required: false },      // Value of milestone achieved
      milestoneAchievedDate: { type: 'date', required: false }   // Date when milestone was achieved
    },
  ...
  PLAYED_AGAINST_OPPONENT: {
    ...
    conditions: 'EXISTS { MATCH (p)-[:PLAYED_IN]->(md:MatchDetail)<-[:HAS_MATCH_DETAILS]-(f:Fixture) WHERE f.opposition = od.opposition }'
  },
};
