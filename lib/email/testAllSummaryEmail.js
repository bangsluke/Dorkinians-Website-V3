...
function e2ePathToArea(filePath, classname) {
	...
	const m = n.match(/(\d{2}-[^/\\]+)/);
	if (m && E2E_FOLDER_LABELS[m[1]]) return E2E_FOLDER_LABELS[m[1]];
	if (m) return m[1];
	return "Other";
}
...
function junitCasesToModel(testcases, suitePassed) {
	...
	for (const tc of testcases) {
		...
		if (!buckets.has(area)) {
			buckets.set(area, { passed: true, tests: [] });
		}
	...
	const subsections = [...buckets.keys()]
		.sort((a, b) => a.localeCompare(b))
		.map((name) => ({ name, passed: buckets.get(name).passed }));
...
function statusPill(passed) {
	...
	return `<span style="display:inline-block;padding:5px 11px;border-radius:999px;font-size:12px;font-weight:700;background:${bg};color:${color};">${label}</span>`;
}
...
function buildSuiteCard(section) {
      ...
      <tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #eaecf0;">
          <span style="display:inline-block;padding:3px 10px;border-radius:999px;font-size:12px;font-weight:700;background:${
			...
			};color:${sub.passed ? "#177245" : "#b42318"};">
	...
	return `
      <div style="margin:16px 0;border:1px solid ${borderColor};border-radius:12px;overflow:hidden;background:#ffffff;">
        <div style="padding:14px 16px;border-bottom:1px solid #eaecf0;background:#f9fafb;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
          <div>
      ...
      <tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #eaecf0;">
          <span style="display:inline-block;padding:3px 10px;border-radius:999px;font-size:12px;font-weight:700;background:${
			...
			};color:${item.result ? "#177245" : "#b42318"};">
 ...
 * @param {{ unit: boolean, integration: boolean, databaseSibling: boolean, databaseSiblingSkipped: boolean, otherJest: boolean, e2e: boolean, chatbotReport: boolean, questionsReport: boolean }} params.suitePass
...
function buildSectionsFromArtifacts(params) {
	...
	const dbPrefix = "../database-dorkinians";
	const dbTestChain = [
		`npm run test:club-records --prefix ${dbPrefix}`,
		`npm run test:graph-insights --prefix ${dbPrefix}`,
		`npm run test:match-derived --prefix ${dbPrefix}`,
		`npm run test:streak-detection --prefix ${dbPrefix}`,
	].join(" && ");
	if (suitePass.databaseSiblingSkipped) {
		sections.push({
			displayName: "Database package (Node)",
			command: `${dbTestChain} (not run - sibling ${dbPrefix} missing)`,
			suitePassed: true,
			subsections: [{ name: "Skipped (sibling repo not checked out)", passed: true }],
		...
		sections.push({
			displayName: "Database package (Node)",
			command: dbTestChain,
			suitePassed: suitePass.databaseSibling,
			subsections: [{ name: "All scripts", passed: suitePass.databaseSibling }],
			counts: {
				passed: suitePass.databaseSibling ? 1 : 0,
				failed: suitePass.databaseSibling ? 0 : 1,
				skipped: 0,
			},
			detailTests: [],
		});
