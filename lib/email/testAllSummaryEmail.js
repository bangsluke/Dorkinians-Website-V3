...
function getTestAllArtifactPaths(repoRoot) {
	// Keep out of Playwright outputDir (__tests__/e2e/test-results) so E2E does not wipe Jest JSON.
	const dir = path.join(repoRoot, "__tests__", "artifacts", "test-all");
	return {
		dir,
...
function filePathToTitle(filePath) {
	...
	return base || "(unnamed suite)";
}

function jestSuiteFailureMessage(tr) {
	if (!tr || typeof tr !== "object") return "";
	const parts = [];
	if (typeof tr.message === "string" && tr.message.trim()) parts.push(tr.message.trim());
	if (Array.isArray(tr.failureMessage) && tr.failureMessage.length) {
		parts.push(tr.failureMessage.filter((x) => typeof x === "string").join("\n").trim());
	}
	if (typeof tr.failureMessage === "string" && tr.failureMessage.trim()) {
		parts.push(tr.failureMessage.trim());
	}
	return excerptText(parts.filter(Boolean).join("\n"), 1200);
}

function parseJestFailureTitlesFromLog(logText) {
	...
	function pushName(raw) {
		const n = String(raw || "").replace(/\u001b\[[0-9;]*m/g, "").trim();
		if (!n) return;
		if (/^Test suite failed to run$/i.test(n)) return;
		if (seen.has(n)) return;
		seen.add(n);
		names.push(n);
	}

	// Prefer FAIL <path> so emails show the real suite file, not the generic ● title.
	for (const line of lines) {
		const m = line.match(/^\s*FAIL\s+(.+)$/);
		if (m && m[1]) {
			pushName(filePathToTitle(m[1]));
		}
	}
...
function applyJestLogFallback(section, suiteLog) {
	if (!section || !suiteLog || section.suitePassed) return section;
	const existing = Array.isArray(section.detailTests) ? section.detailTests : [];
	if (existing.length > 0) return section;

	const parsedNames = parseJestFailureTitlesFromLog(suiteLog);
	if (parsedNames.length === 0) return section;

	const detailTests = parsedNames.map((title) => ({
		title,
		status: "failed",
		durationSec: null,
		message: "",
	}));
	const baseCounts = section.counts || { passed: 0, failed: 0, skipped: 0 };
	return {
		...section,
		counts: { ...baseCounts, failed: Math.max(baseCounts.failed || 0, detailTests.length) },
		detailTests,
	};
}
...
function subsectionStatus(subsection) {
	if (subsection && typeof subsection.status === "string") {
		return normalizeStatusValue(subsection.status);
	}
	if (subsection && subsection.passed === true) return "passed";
	if (subsection && subsection.passed === false) return "failed";
	return "failed";
}
...
function jestJsonToModel(jestJson, mode, suitePassed) {
	...
	if (jestJson && Array.isArray(jestJson.testResults)) {
		for (const tr of jestJson.testResults) {
			...
			if (!buckets.has(bname)) {
				buckets.set(bname, { hasPassed: false, hasFailed: false, tests: [] });
			}
			const bucket = buckets.get(bname);
			const assertions = toArray(tr.assertionResults);
			const suiteMsg = jestSuiteFailureMessage(tr);
			for (const ar of assertions) {
				...
				if (isAssertionOk(st)) bucket.hasPassed = true;
				else if (!isSkippedLike(st)) bucket.hasFailed = true;
			}

			// Some Jest failures (setup/runtime/import errors) can fail a suite with no assertionResults.
			// Emit a synthetic failed row so emails still show where to investigate.
			if (assertions.length === 0 && (tr.status === "failed" || suiteMsg)) {
				bucket.tests.push({
					title: filePathToTitle(file),
					status: "failed",
					durationMs: null,
					message: suiteMsg,
				});
				bucket.hasFailed = true;
			}
		}
	}
	...
	const subsections = [];
	if (buckets.size === 0) {
		subsections.push({ name: "Summary", status: suitePassed ? "passed" : "failed" });
	} else {
		const names = [...buckets.keys()].sort((a, b) => a.localeCompare(b));
		for (const name of names) {
			const bucket = buckets.get(name);
			const status = bucket.hasFailed ? "failed" : bucket.hasPassed ? "passed" : "skipped";
			subsections.push({ name, status, passed: status === "passed" });
		}
	}
...
function junitCasesToModel(testcases, suitePassed) {
	if (!testcases || testcases.length === 0) {
		return {
			subsections: [{ name: "Summary", status: suitePassed ? "passed" : "failed" }],
			counts: { passed: 0, failed: 0, skipped: 0 },
			detailTests: [],
		};
	}

	/** @type {Map<string, { label: string, hasPassed: boolean, hasFailed: boolean, tests: object[] }>} */
	const buckets = new Map();
	...
	for (const tc of testcases) {
		...
		if (!buckets.has(area.key)) {
			buckets.set(area.key, { label: area.label, hasPassed: false, hasFailed: false, tests: [] });
		}
		...
		if (status === "passed") bucket.hasPassed = true;
		else if (status === "failed") bucket.hasFailed = true;
	}
	...
	const subsections = orderedKeys.map((key) => {
		const bucket = buckets.get(key);
		const status = bucket.hasFailed ? "failed" : bucket.hasPassed ? "passed" : "skipped";
		return { name: bucket.label, status, passed: status === "passed" };
	});
...
function statusPill(passed) {
	...
	return `<span style="display:inline-block;white-space:nowrap;word-break:normal;line-height:12px;mso-line-height-rule:exactly;padding:5px 11px;border-radius:999px;font-size:12px;font-weight:700;background:${bg};color:${color};">${label}</span>`;
}
...
function buildSuiteCard(section) {
	...
	const subRows = section.subsections
		.map(
			(sub) => {
				const status = subsectionStatus(sub);
				const bg = status === "passed" ? "#ecfdf3" : status === "skipped" ? "#fffbeb" : "#fef3f2";
				const color = status === "passed" ? "#177245" : status === "skipped" ? "#b45309" : "#b42318";
      ...
      <tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #eaecf0;white-space:nowrap;">
          <span style="display:inline-block;padding:3px 10px;border-radius:999px;font-size:12px;font-weight:700;background:${bg};color:${color};white-space:nowrap;line-height:1;">
            ${label}
          </span>
        </td>
      </tr>`;
	...
	return `
      <div style="margin:16px 0;border:1px solid ${borderColor};border-radius:12px;overflow:hidden;background:#ffffff;">
        <div style="padding:14px 16px;border-bottom:1px solid #eaecf0;background:#f9fafb;">
          <table role="presentation" style="border-collapse:collapse;width:100%;">
            <tr>
              ...
              <td style="vertical-align:top;padding:0 0 0 8px;text-align:right;white-space:nowrap;width:1%;">
...
function buildDetailListHtml(section) {
	...
	const needsFailureFallback = !hasTests && c.failed > 0;
	...
	if (needsFailureFallback) {
		body += `
		<div style="margin:10px 0 0;padding:12px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;">
			<div style="font-size:12px;font-weight:700;color:#b42318;">Failing test details unavailable</div>
			<div style="font-size:12px;color:#7a271a;margin-top:4px;">This suite failed, but specific test names were not available from artifacts. Check the CI job logs/artifacts for exact failing tests.</div>
		</div>`;
	}
 ...
 * @param {{ unit?: string, integration?: string, otherJest?: string, chatbot?: string, questions?: string }} params.logs
 */
function buildSectionsFromArtifacts(params) {
	...
	sections[sections.length - 1] = applyJestLogFallback(sections[sections.length - 1], logs?.unit);
	...
	sections[sections.length - 1] = applyJestLogFallback(sections[sections.length - 1], logs?.integration);
	...
	sections[sections.length - 1] = applyJestLogFallback(sections[sections.length - 1], logs?.otherJest);
...
function buildTestAllEmailPlainText(opts) {
	...
	for (const section of sections) {
		...
		for (const sub of section.subsections) {
			lines.push(`  - ${sub.name}: ${subsectionStatus(sub).toUpperCase()}`);
		}
		const c = section.counts;
		...
		if (section.detailTests && section.detailTests.length > 0) {
			...
			lines.push("    * [failed] Failing test details unavailable in artifacts; check CI logs/artifacts for specific test names.");
		}
