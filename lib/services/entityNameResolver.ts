...
export class EntityNameResolver {
	 ...
	 */
	public async resolveEntityName(input: string, entityType: 'player' | 'team' | 'opposition' | 'league' | 'stat_type'): Promise<EntityResolutionResult> {
		// Normalize input
		...
		try {
			...
			switch (entityType) {
				...
				case 'stat_type':
					...
					const statTypeNames = [
						'Apps', 'Minutes', 'Man of the Match', 'Goals', 'Open Play Goals',
						'Assists', 'Yellow Cards', 'Red Cards', 'Saves', 'Own Goals',
						'Goals Conceded', 'Clean Sheets', 'Penalties Scored', 'Penalties Missed',
						'Penalties Conceded', 'Penalties Saved', 'Fantasy Points', 'Goal Involvements',
						'Goals Per Appearance', 'Conceded Per Appearance', 'Minutes Per Goal',
						'Team of the Week', 'Season Team of the Week', 'Player of the Month',
						'Captain', 'Captain Awards', 'Home', 'Away'
					];
	...
	private findExactMatch(input: string, entities: string[]): string | undefined {
		...
		const normalizedMatch = entities.find(entity => 
			this.normalizeEntityName(entity) === input
		);
	 ...
	 */
	private findFuzzyMatches(input: string, entities: string[], entityType: 'player' | 'team' | 'opposition' | 'league' | 'stat_type'): EntityMatch[] {
	...
	private generateSuggestions(input: string, entities: string[]): string[] {
		...
		const startsWithMatches = entities.filter(entity => 
			entity.toLowerCase().startsWith(input.toLowerCase()) ||
			input.toLowerCase().startsWith(entity.toLowerCase())
		);
		...
		if (suggestions.length < this.MAX_SUGGESTIONS) {
			const partialMatches = entities.filter(entity => 
				entity.toLowerCase().includes(input.toLowerCase()) ||
				input.toLowerCase().includes(entity.toLowerCase())
			);
