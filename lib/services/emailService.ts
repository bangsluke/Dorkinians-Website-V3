...
`;
	...
	private generateSeedingSummaryEmail(summary: SeedingSummary): string {
		let html = `
      <h2>🌱 Database Seeding Summary</h2>
      <p>The database seeding process for environment <strong>${summary.environment.toUpperCase()}</strong> has completed:</p>
      <p><strong>Time:</strong> ${new Date().toISOString()}</p>
      
      <h3>📊 Summary:</h3>
      <ul>
        <li><strong>Environment:</strong> ${summary.environment}</li>
        <li><strong>Nodes Created:</strong> ${summary.nodesCreated}</li>
        <li><strong>Relationships Created:</strong> ${summary.relationshipsCreated}</li>
        <li><strong>Duration:</strong> ${summary.duration} seconds</li>
        <li><strong>Error Count:</strong> ${summary.errorCount}</li>
        <li><strong>Success:</strong> ${summary.success ? "✅ Yes" : "⚠️ No"}</li>
      </ul>
    `;
      ...
      <h3>🔧 Required Actions:</h3>
      <ol>
        <li>Review the database for any missing or incorrect data.</li>
...
The database seeding process for environment ${summary.environment.toUpperCase()} has completed:
...
REQUIRED ACTIONS:
1. Review the database for any missing or incorrect data.
