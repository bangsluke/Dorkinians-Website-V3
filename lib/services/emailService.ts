				...
				<div class="container">
					...
					<div class="content">
						...
						${
							failureCypher
								? `
							<div class="error-section" style="background: #fff8e1; border-color: #ffe082;">
								<h3 style="color: #856404; margin-top: 0;">Neo4j Debug Query</h3>
								<p style="margin-bottom: 10px;">Copy and paste this into Neo4j Aura Browser to inspect the failure:</p>
...
`;
		...
		if (failureCypher) {
			text += `NEO4J DEBUG QUERY:\n`;
			text += `Copy and paste into Neo4j Aura Browser:\n`;
			text += `${failureCypher}\n\n`;
		}
	...
	private getFailureCypherFromSummary(summary: SeedingSummary): string | null {
		const combinedText = [
			...(Array.isArray(summary.errors) ? summary.errors : []),
			String((summary as { error?: string }).error || ""),
			String((summary as { errorMessage?: string }).errorMessage || ""),
		]
			.join(" ")
			.toLowerCase();
