...
// Updated to use unified schema via schema bridge

const { getCSVHeaderConfig, hasUnifiedSchema } = require('../config/schemaBridge');
...
class CSVHeaderValidator {
	...
	getExpectedHeaders(sourceName) {
		// Try to use unified schema first
		if (hasUnifiedSchema()) {
			const config = getCSVHeaderConfig(sourceName);
			if (config && config.expectedHeaders) {
				return config.expectedHeaders;
			}
		}
