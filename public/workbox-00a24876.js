define(["exports"], function (t) {
		...
		findMatchingRoute({ url: t, sameOrigin: e, request: s, event: n }) {
			...
			for (const r of i) {
				...
				if (a)
					return (
						(i = a),
						((Array.isArray(i) && 0 === i.length) || (a.constructor === Object && 0 === Object.keys(a).length) || "boolean" == typeof a) &&
							(i = void 0),
						{ route: r, params: i }
					);
			}
		...
		async q(t, e, n) {
			...
			try {
				...
				if (s instanceof Error)
					for (const r of t.iterateCallbacks("handlerDidError")) if (((i = await r({ error: s, event: n, request: e })), i)) break;
				if (!i) throw s;
			}
	...
	class A {
		...
		async expireEntries(t, e) {
			...
			for (; n; ) {
				const s = n.value;
				(s.cacheName === this.I && ((t && s.timestamp < t) || (e && r >= e) ? i.push(n.value) : r++), (n = await n.continue()));
			}
			const a = [];
	...
	class X extends v {
		...
		A() {
			...
			for (const [s, n] of this.plugins.entries())
				n !== X.copyRedirectedCacheableResponsesPlugin && (n === X.defaultPrecacheCacheabilityPlugin && (t = s), n.cacheWillUpdate && e++);
	...
	class Y {
		constructor({ cacheName: t, plugins: e = [], fallbackToNetwork: s = !0 } = {}) {
			((this.F = new Map()),
				(this.H = new Map()),
				(this.$ = new Map()),
				(this.u = new X({ cacheName: w(t), plugins: [...e, new V({ precacheController: this })], fallbackToNetwork: s })),
