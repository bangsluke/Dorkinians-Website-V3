...
// TOTW / Players-of-Month: listbox helpers tolerate Headless UI portals; many assertions are skipped on Mobile Chrome
// (known flaky UI/data). Desktop tests still skip when selectors or API data are insufficient.
...
test.describe("TOTW Page Tests", () => {
	...
	test("4.2. should display TOTW page by default", async ({ page }, testInfo) => {
		...
		if (mobile) test.skip(true, "Mobile TOTW UI/data is not stable enough for deterministic assertions.");
		await navigateToMainPage(page, "totw");
	...
	test("4.12. with a globally selected player, that player should be highlighted in both This Month and This Season FTP ranking tables when present", async ({
		...
		if (!(await monthSection.isVisible({ timeout: 8000 }).catch(() => false))) {
			test.skip(true, "Month FTP column not visible - skipping.");
			return;
		}
		if (!(await seasonSection.isVisible({ timeout: 8000 }).catch(() => false))) {
			test.skip(true, "Season FTP column not visible - skipping.");
			return;
		}
	...
	test("4.13. rank 1 in This Month FTP Ranking should also appear in This Season FTP Ranking when both tables have data", async ({ page }, testInfo) => {
		...
		if (!(await monthTable.isVisible({ timeout: 12000 }).catch(() => false))) {
			test.skip(true, "Month FTP ranking table not visible - skipping rank-1 cross-check.");
			return;
		}
