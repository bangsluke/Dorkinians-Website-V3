...
export const FALLBACK_TEST_DATA: TestPlayerData[] = [
  {
    ...
    penaltiesSaved: 0,
    allGoals: 29,
    goalsPerAppearance: 0.37,
    concededPerAppearance: 0,
    minutesPerGoal: 242,
    minutesPerCleanSheet: 2340,
    fantasyPointsPerAppearance: 2.0,
    distance: 0,
    homeGames: 0,
    homeWins: 0,
  ...
  },
  {
    ...
    penaltiesSaved: 0,
    allGoals: 15,
    goalsPerAppearance: 0.33,
    concededPerAppearance: 0,
    minutesPerGoal: 270,
    minutesPerCleanSheet: 0,
    fantasyPointsPerAppearance: 2.2,
    distance: 0,
    homeGames: 0,
    homeWins: 0,
    homeGamesPercentWon: 0,
  ...
  },
  {
    ...
    penaltiesSaved: 0,
    allGoals: 8,
    goalsPerAppearance: 0.15,
    concededPerAppearance: 0,
    minutesPerGoal: 585,
    minutesPerCleanSheet: 0,
    fantasyPointsPerAppearance: 1.7,
    distance: 0,
    homeGames: 0,
    homeWins: 0,
    homeGamesPercentWon: 0,
...
export function generateTestQuestions(playerName: string): string[] {
  return STAT_TEST_CONFIGS.map(config => 
    config.questionTemplate.replace('{playerName}', playerName)
  );
}
...
export async function fetchTestData(): Promise<TestPlayerData[]> {
  ...
  try {
    ...
    const processedData = parsed.data.map((row: any) => ({
      ...
      homeGames: Number(row.homeGames || row.HomeGames || row.HomeGames || 0),
      homeWins: Number(row.homeWins || row.HomeWins || row.HomeWins || 0),
      homeGamesPercentWon: Number(row.homeGamesPercentWon || row.HomeGamesPercentWon || row.HomeGamesPercentWon || 0),
      awayGames: Number(row.awayGames || row.AwayGames || row.AwayGames || 0),
      awayWins: Number(row.awayWins || row.AwayWins || row.AwayWins || 0),
      awayGamesPercentWon: Number(row.awayGamesPercentWon || row.AwayGamesPercentWon || row.AwayGamesPercentWon || 0),
    ...
    if (isVerbose) {
      console.warn('⚠️ Failed to fetch reference data from CSV, using fallback data:', error);
    }
    
    // Return fallback data if CSV fetch fails
