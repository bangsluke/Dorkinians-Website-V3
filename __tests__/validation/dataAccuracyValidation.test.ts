    ...
    test('should validate basic stats against reference data', async () => {
      ...
      for (const playerData of referenceData) {
        for (const statKey of basicStats) {
          ...
          switch (statKey) {
            case 'G': expectedValue = playerData.goals; break;
            case 'A': expectedValue = playerData.assists; break;
            case 'APP': expectedValue = playerData.appearances; break;
            case 'Y': expectedValue = playerData.yellowCards; break;
            case 'R': expectedValue = playerData.redCards; break;
            case 'FTP': expectedValue = playerData.fantasyPoints; break;
            default: expectedValue = 0;
          }
    ...
    test('should validate advanced stats against reference data', async () => {
      ...
      for (const playerData of referenceData) {
        for (const statKey of advancedStats) {
          ...
          let expectedValue: number;
          switch (statKey) {
            case 'GperAPP': expectedValue = playerData.goalsPerAppearance; break;
            case 'CperAPP': expectedValue = playerData.concededPerAppearance; break;
            case 'MperG': expectedValue = playerData.minutesPerGoal; break;
            case 'MperCLS': expectedValue = playerData.minutesPerCleanSheet; break;
            case 'FTPperAPP': expectedValue = playerData.fantasyPointsPerAppearance; break;
            default: expectedValue = 0;
          }
