...
/** Local monorepo sibling or CI checkout under website root. */
function resolveMatchDerivedFieldsPath(): string | null {
	const candidates = [
		path.join(__dirname, "../../../database-dorkinians/services/matchDerivedFields.js"),
		path.resolve(process.cwd(), "database-dorkinians/services/matchDerivedFields.js"),
		path.resolve(process.cwd(), "../database-dorkinians/services/matchDerivedFields.js"),
	];
	return candidates.find((p) => fs.existsSync(p)) ?? null;
}

type MatchDerivedFieldsModule = {
...
const dbRoot = resolveMatchDerivedFieldsPath();

if (!dbRoot) {
	describe.skip("matchDerivedFields (shared with database-dorkinians)", () => {
	...
	} = require(dbRoot) as MatchDerivedFieldsModule;
