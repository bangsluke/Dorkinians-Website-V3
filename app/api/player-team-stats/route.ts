...
export async function POST(request: NextRequest) {
	try {
		...
		// Build filter conditions (excluding team filter since we're grouping by team)
		const filterConditions = filters ? buildFilterConditions(filters, params) : [];
		const conditionsWithoutTeam = filterConditions.filter(cond => !cond.includes("f.team") && !cond.includes("md.team") && !cond.includes("teams"));
		if (conditionsWithoutTeam.length > 0) {
			query += ` WHERE ${conditionsWithoutTeam.join(" AND ")}`;
		}

		// Group by team and aggregate stats
		query += `
			WITH md.team as team, md
		...
		const teamStats = result.records.map((record: Record) => ({
		...
		return NextResponse.json({ teamStats }, { headers: corsHeaders });
	} catch (error) {
		console.error("Error fetching player team stats:", error);
		return NextResponse.json({ error: "Failed to fetch player team stats" }, { status: 500, headers: corsHeaders });
	}
}
