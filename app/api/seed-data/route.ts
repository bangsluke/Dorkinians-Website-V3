    ...
    if (action === 'validate-headers-only') {
      ...
      try {
        ...
        if (!headerValidationResult.isValid) {
          console.error(`❌ CSV header validation failed. ${headerValidationResult.failedSources} out of ${headerValidationResult.totalSources} data sources have invalid headers.`)
