...
const STAT_FIELD_MAP: Record<StatType, string> = {
...
function mapRecordToPlayer(record: Record, statType: StatType) {
	const statField = STAT_FIELD_MAP[statType];
	const player = {
	...
	console.log(
		`[TopPlayersStats] Player: ${player.playerName}, ${statType}: ${player[statField as keyof typeof player]}, statType: ${statType}`
	);
	return player;
}
...
function buildAggregationQuery(
	...
	options?: { playerNameFilter?: string }
): { query: string; sortField: string; sortDirection: "ASC" | "DESC" } {
	const statField = STAT_FIELD_MAP[statType];
	const sortField = statType === "goals" ? "totalGoals" : statField;
	const sortDirection: "ASC" | "DESC" = ["concededPer90", "cardsPer90"].includes(statType) ? "ASC" : "DESC";
	...
	if (options?.playerNameFilter) {
		params.highlightPlayerName = options.playerNameFilter;
		query += ` AND p.playerName = $highlightPlayerName`;
	}

	return { query, sortField, sortDirection };
}

function parseLimit(limit: unknown): number | "all" | null {
	if (limit === undefined || limit === null) return 5;
	if (limit === "all") return "all";
	if (typeof limit === "number" && Number.isInteger(limit) && limit >= 1 && limit <= 500) return limit;
	return null;
}
...
export async function POST(request: NextRequest) {
	try {
		...
		const { filters, statType, limit: rawLimit, highlightPlayerName } = body;

		if (!statType || !VALID_STAT_TYPES.includes(statType)) {
			return NextResponse.json(
				{ error: `Valid statType is required. Options: ${VALID_STAT_TYPES.join(", ")}` },
		...
		if (parsedLimit === null) {
			return NextResponse.json(
				{ error: "limit must be a number between 1 and 500, or 'all'" },
				{ status: 400, headers: corsHeaders }
			);
		}
		...
		const { query: baseQuery, sortField, sortDirection } = buildAggregationQuery(filters, statType, params);
		let query = baseQuery + RETURN_FIELDS + `\n\t\t\tORDER BY ${sortField} ${sortDirection}, appearances ASC`;
		...
		const players = result.records.map((record: Record, index: number) => ({
			...mapRecordToPlayer(record, statType),
			rank: index + 1,
		}));

		let highlightPlayer: { player: ReturnType<typeof mapRecordToPlayer>; rank: number } | null = null;
		let totalCount: number | undefined;

		if (typeof highlightPlayerName === "string" && highlightPlayerName.trim()) {
			const trimmedName = highlightPlayerName.trim();
			const inResults = players.some((p) => p.playerName === trimmedName);

			if (!inResults) {
				const rankParams: Record<string, unknown> = { graphLabel };
				const { query: rankBaseQuery, sortField: rankSortField, sortDirection: rankSortDirection } =
					buildAggregationQuery(filters, statType, rankParams);
				const rankQuery =
					rankBaseQuery +
					`\n\t\t\tRETURN p.playerName as playerName, ${rankSortField} as sortValue, appearances as appearances` +
					`\n\t\t\tORDER BY ${rankSortField} ${rankSortDirection}, appearances ASC`;

				const rankResult = await neo4jService.runQuery(rankQuery, rankParams);
				totalCount = rankResult.records.length;

				const highlightIndex = rankResult.records.findIndex(
					(record: Record) => String(record.get("playerName") || "") === trimmedName
				);

				if (highlightIndex >= 0) {
					const highlightParams: Record<string, unknown> = { graphLabel };
					const { query: highlightBaseQuery } = buildAggregationQuery(filters, statType, highlightParams, {
						playerNameFilter: trimmedName,
					});
					const highlightQuery = highlightBaseQuery + RETURN_FIELDS;
					const highlightResult = await neo4jService.runQuery(highlightQuery, highlightParams);

					if (highlightResult.records.length > 0) {
						highlightPlayer = {
							player: mapRecordToPlayer(highlightResult.records[0], statType),
							rank: highlightIndex + 1,
						};
					}
				}
			}
