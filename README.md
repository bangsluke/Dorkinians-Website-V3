...
- [Testing Commands and Coverage](#testing-commands-and-coverage)
  - [CI on main and email](#ci-on-main-and-email)
...
**If the live site returns 500 for every URL (including `/favicon.ico`):** You are on **Netlify** (this Next.js app), not the Heroku **seeder** (`database-dorkinians`). In **Netlify → Logs**, search for `Environment variable validation failed` from `app/layout.tsx` / `lib/config/envValidation.ts`. Ensure required keys exist in the Netlify UI and include **Builds** and **Functions** scopes where offered. Production builds run `scripts/sync-build-env-to-dotenv.cjs` before `next build` (see `netlify.toml`) so those values are bundled for the Next server handler.
...
**Pushes to `main`**, a **weekly schedule** (Tuesday 02:17 UTC), and manual **workflow dispatch** run the GitHub Actions workflow [`.github/workflows/full-test-suite-and-email.yml`](./.github/workflows/full-test-suite-and-email.yml). The job installs dependencies, installs Playwright Chromium, runs **`npm run test:all`** with **`SEND_CI_TEST_ALL_SUMMARY_EMAIL`**, then sends **one SMTP email** with the subject and layout described above (per-suite breakdown and fail/skip detail when applicable). **`CI=true`** in the workflow enables Playwright’s JUnit file for E2E line items in that email. Artifacts: Playwright HTML report, screenshots, and `test-all-artifacts` (Jest JSON + JUnit).

Configure repository **Actions secrets**: `SMTP_SERVER`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM_EMAIL`, `SMTP_TO_EMAIL`; optional `SMTP_EMAIL_SECURE`, `WEBSITE_URL` (E2E target), and **`CROSS_REPO_CHECKOUT_TOKEN`** (PAT with read access to private `bangsluke/Database-Dorkinians` so CI can run badge catalog / matchDerivedFields unit parity; without it those suites skip cleanly).

This workflow is **informational by default**: it does not block merges to `main` unless you add it as a **required status check** under branch protection. Failed runs appear in the **Actions** tab; you can also enable **Actions** notifications in your GitHub account settings for extra alerts.
