...
export default function LeagueResultsModal({ isOpen, onClose, teamKey, teamDisplayName, season }: LeagueResultsModalProps) {
	const [fixtures, setFixtures] = useState<Fixture[]>([]);
	const [loading, setLoading] = useState(false);
	const [error, setError] = useState<string | null>(null);
	const [expandedFixtureId, setExpandedFixtureId] = useState<string | null>(null);
	const [lineupByFixtureId, setLineupByFixtureId] = useState<Record<string, FixtureLineupPlayer[]>>({});
	...
	useEffect(() => {
		...
		const fetchFixtures = async () => {
			...
			setError(null);
			try {
				const response = await fetch(`/api/league-fixtures?team=${encodeURIComponent(teamKey)}&season=${encodeURIComponent(season)}`);
				if (response.ok) {
	...
	const formatGoalscorers = (goalscorers: Goalscorer[] | undefined, oppoOwnGoals: number = 0): string => {
		...
		if (goalscorers && Array.isArray(goalscorers) && goalscorers.length > 0) {
			const validGoalscorers = goalscorers.filter((g) => g && g.playerName && Number(g.goals) > 0);
			...
			<div className='h-full flex flex-col' style={{ backgroundColor: "#0f0f0f" }}>
				...
				<div className='flex-1 overflow-y-auto p-4' style={{ WebkitOverflowScrolling: "touch" }}>
					...
					{!loading && !error && fixtures.length === 0 && (
						<div className='text-center text-gray-400 py-8'>No league fixtures found for this team and season</div>
					)}
					...
					{!loading && !error && fixtures.length > 0 && (
						<div className='space-y-4'>
							{fixtures.map((fixture, index) => {
								...
								const goalscorersText = formatGoalscorers(fixture.goalscorers, fixture.oppoOwnGoals || 0);
								return (
									<div key={fixtureId || index} className='bg-gray-800/50 rounded-lg border border-white/10 overflow-hidden'>
										<button
											type='button'
											...
											<div className='absolute top-4 right-4 flex gap-2 items-center'>
												...
												<span
													className={`px-2 py-1 rounded text-xs font-medium ${
														fixture.homeOrAway?.toLowerCase() === "home" ? "bg-dorkinians-yellow/20 text-dorkinians-yellow" : "bg-gray-700 text-gray-300"
													}`}>
