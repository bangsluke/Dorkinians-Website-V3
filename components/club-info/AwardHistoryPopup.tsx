...
export default function AwardHistoryPopup({ playerName, onClose }: AwardHistoryPopupProps) {
	...
	useEffect(() => {
		const fetchHistory = async () => {
			setLoading(true);
			try {
				const response = await fetch(`/api/awards/player-history?playerName=${encodeURIComponent(playerName)}`);
				if (!response.ok) {
		...
		>
			...
			>
				...
				<div className='flex-shrink-0 flex justify-between items-center p-4 border-b border-white/20'>
					...
					<button onClick={onClose} className='text-white hover:text-gray-200 ml-4 flex-shrink-0' aria-label={`Close ${playerName} award history`}>
				...
				<div className='flex-shrink-0 text-center mb-4 pt-4'>
					<p className='text-white text-lg md:text-xl font-bold'>
						Total Awards: <span className='text-dorkinians-yellow'>{totalAwards}</span>
					</p>
				</div>
